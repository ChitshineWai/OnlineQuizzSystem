        ...
        <div class="container">
            <div class="row d-flex align-items-center py-5">
                <div class="col-lg-6 text-start">
                    ...
                    $q_id = $_GET['q_id'];
                    $selc = "SELECT * FROM html_question WHERE q_id='$q_id'";
                    $r = $con->query($selc);
                    $row = mysqli_fetch_array($r);
                    ?>
                    <h1 class="h2 py-5 text-primary typo-space-line"><?php echo $row['question']?></h1>
                    <form action="htmlprocess.php" method="post">
                    <?php
                    $selcc = "SELECT * FROM html_choice WHERE q_id='$q_id' ORDER BY rand()";
                    $rr = $con->query($selcc);
                    while(list($id,$q_id,$true_ans,$choice) = mysqli_fetch_array($rr)){
                        echo " 
                          <div class='form-check'><input class='form-check-input h5' type='radio' name='quiz' id='exampleRadios1' 
                        value='$true_ans'>
                        <label class='form-check-label h5' style='padding-left:30px;margin-bottom:5px;'>
                       $choice
                        </label></div>";
                      }
                    ?>
                    <button type="submit" name="next_q" class="btn btn-primary m-2 float-end">next question</button>
                    <input type="hidden" name="h_number" value="<?php echo  $_GET['q_id']; ?>">
                    </form>
                </div>
                <div class="col-lg-6">
